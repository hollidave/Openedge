...
USING CaseFlow.Helpers.StringHelper. 
...
DEFINE VARIABLE gicRequestID    AS CHARACTER NO-UNDO.
...
FUNCTION fnSquare RETURNS LOGICAL 
    ( INPUT iCheckRow AS INTEGER, INPUT iCheckCol AS INTEGER, INPUT iActualRow AS INTEGER, INPUT iActualCol AS INTEGER, INPUT iCorner AS INTEGER ):
/*------------------------------------------------------------------------------
 Purpose:
 Notes:
------------------------------------------------------------------------------*/    
    ...
    IF AVAILABLE ttBoard AND ttBoard.board[iCorner] = giOpponent THEN 
        lSkip = FALSE.
...
gcBestCounters = StringHelper:Concat( gcBestCounters, STRING(giBestX) + "," + STRING(giBestY), "|" ). 
    ...
    IF AVAILABLE bttPosition THEN DO: 
        IF bttPosition.iValue = giOpponent THEN   
            ASSIGN ioiCounter = ioiCounter + 1
                   iocCounters = StringHelper:Concat( iocCounters, STRING(iiColumn) + "," + STRING(iiRow), "|" ). 
...
ASSIGN cSourceType = "file" 
       cReadMode   = "empty".
ASSIGN gcGameGUID = "f086347a-c2bf-11ea-b997-005056bb84e6"
       gicRequestID = gcGameGUID.
 
OUTPUT STREAM strOutput TO VALUE( "H:\20.07\Othello\Log-TESTER.log" ).

PUT STREAM strOutput UNFORMATTED "Game Started".

ASSIGN gcPlayerName = "David".

ASSIGN cFile       = "H:\20.07\Othello\game-" + gcGameGUID + ".json"
       cFileOut    = "H:\20.07\Othello\move-" + gicRequestID + ".json". 
    ...
    IF ttBoard.winner <> "" THEN DO:
        IF ttBoard.winner = gcPlayerName THEN 
            PUT STREAM strOutput UNFORMATTED "Yay. I Won".
        ELSE DO:
            PUT STREAM strOutput UNFORMATTED "Boo, I Lost". 
        END.
        MESSAGE "Ended"
        VIEW-AS ALERT-BOX. 
    END.
